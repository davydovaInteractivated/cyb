...
const Calculate = ({ t }: { t: any }) => {
  ...
  const { services, activeService, getService } = useContext(ServicesContext);
  ...
  const { servicesCases, getServicesCases, cases, setCases, getTotalCost } = useContext(CasesContext);
  ...
  useEffect(() => {
    const aCase = servicesCases.find((cs: TServiceCase) => cs.id === currentServiceId) || null;
        ...
        {activeCase && (
          <div className="calculate--page__form">
            ...
            <div className="calculate--page__form-item flex gap-2 align-center justify-end">
              <strong>
                {t('total')}:&nbsp;&nbsp;~&nbsp;${getTotalCost(currentCase)}
              </strong>
              <CustomButton onClick={addCase}>{t('addtocase')}</CustomButton>
            </div>
          </div>
        )}
      </div>
