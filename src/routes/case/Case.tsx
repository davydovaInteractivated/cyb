...
/** Contexts */
import { CasesContext, TCase, TCaseDataKey, TCaseDataField } from '../../context/cases.context';
...
const Case = ({ caseItem, caseIndex, t }: ICaseProps) => {
  ...
  const deleteCase = () => {
    ...
    const filteredCases = cases.filter((_, index) => caseIndex !== index);
  ...
  const getDownloadData = () => {
    ...
    const data: TDataItem[][] = dataValues.map((value: TCaseDataField, index: number) => [
      {
  ...
  const downloadCase = async () => {
    ...
    tableDataToPdf(`${caseItem.title}-case.pdf`, 'p', { data, headers });
  };
    ...
    <div className="case">
      <div className="case--header flex align-center justify-space-b">
        <h2 className="case--title">
          ...
          {t('case.pre-calculate')} {t('case.case')}
        </h2>
        <CustomButton icon onClick={() => deleteCase()}>
          <XMarkIcon className="case--delete__icon" />
        </CustomButton>
      </div>
      <div className="case--content grid gap">
        {Object.keys(data).map((key) => (
          <div key={key} className="case--content__field">
        ...
        <div className="case--content__footer w-100 grid gap">
          <strong className="case--content__total">
            {t('total')}:&nbsp;&nbsp;~&nbsp;${getTotalCost(caseItem)}
          </strong>
          <CustomButton className="case--content__download" size="small" onClick={() => downloadCase()}>
            {t('download')} PDF
          </CustomButton>
        </div>
      </div>
