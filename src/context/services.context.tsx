import React, { PropsWithChildren, useCallback, createContext, useState, useEffect, useContext } from 'react';
 ...
 */
const getService = (Id: string | null, services: TService[]): TService | null =>
  services.find((serv) => serv.id === Id) || null;
...
export const ServicesContextProvider = ({ children }: PropsWithChildren) => {
  ...
  const filter = useCallback(
    ({
      ...
      /** Search Cards */
      if (searchValue) {
        filteredServices = filteredServices.filter((service: TService) =>
          service.title[activeLang as TLangsKeys].toLocaleLowerCase().includes(searchValue)
        );
      }
        ...
        filteredServices =
          sortDirection === 1
            ? [...(filteredServices || [])].sort((a, b) =>
                a.title[activeLang as TLangsKeys]
                  .toLowerCase()
                  .localeCompare(b.title[activeLang as TLangsKeys].toLowerCase())
              )
            : [...(filteredServices || [])].sort((a, b) =>
                b.title[activeLang as TLangsKeys]
                  .toLowerCase()
      ...
      if (onFavorites) {
        filteredServices = filteredServices.filter((service) => service.is_marked);
      }
  ...
  const markService = (isMarked: boolean, serviceId: string) => {
    ...
    const currentServiceDataIndex = servicesDataCopy.findIndex((card) => card.id === serviceId);
