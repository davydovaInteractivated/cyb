...
const Cube = ({
  ...
  return (
    <CubeStyled className={`cube ${className}`} $type={type} $animationType={animationType} $speed={speed} $size={size}>
      {cubeData.map((side: TSideCube) => (
        <span key={side.name} className={`cube--side flex align-center justify-center ${side.class}`}>
          {showText && t(`cube.side.${side.name}`)}
        </span>
      ))}
    </CubeStyled>
  );
