...
const NavFilters = ({ t, className = '' }: { t: any; className?: string }) => {
  const { sortDirection, searchValue, search, sort } = useContext(ServicesContext);

  return (
    <div className={`nav nav--filters flex align-center ${className}`}>
      <div className="nav__list-item nav--icons nav--icons__sort">
        <CustomButton className={!sortDirection ? '' : 'none m-0'} icon onClick={sort}>
          <ArrowsUpDownIcon className="icons__icon" />
        </CustomButton>
        <CustomButton className={sortDirection > 0 ? 'active' : 'none m-0'} icon onClick={sort}>
